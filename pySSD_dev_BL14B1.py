...
#elif sys.platform == 'darwin' and screen_resolution.height() == 800:
#    from UI_pySSD_lowReso import Ui_MainWindow
else:
...
class params:
    ...
    grid8 = QtGui.QGridLayout()
    grid9 = QtGui.QGridLayout()
...
class MainWindow(QtGui.QMainWindow):
    def __init__(self, parent=None):
            ...
            if len(params.xanes) != 0:
                ...
                for cb in params.xanes:
                    if cb.isChecked():
                        ...
                        func_type = self.findChild(QtGui.QComboBox,cb.objectName()+'_cmb').currentIndex()
                        #print fit_e
        ...
        def set_dialog_parameters():
            ...
            comboBox = self.findChild(QtGui.QComboBox,params.xanes[index].objectName()+'_cmb')
        ...
        def plot_xanes_in_dialog():
            ...
            if len(params.xanes) != 0:
                ...
                # self.findChild(QtGui.QDoubleSpinBox, cb.objectName() + 'dSB_L').setValue(fit_s)
                # self.findChild(QtGui.QDoubleSpinBox, cb.objectName() + 'dSB_H').setValue(fit_e)
                # self.findChild(QtGui.QComboBox, cb.objectName() + '_cmb').setCurrentIndex(func_type)
                # params.xanes_data[os.path.basename(file)][0] = self.plot_dialog.doubleSpinBox_LE.value()
                # params.xanes_data[os.path.basename(file)][1] = self.plot_dialog.doubleSpinBox_HE.value()
                # params.xanes_data[os.path.basename(file)][2] = self.plot_dialog.comboBox_Ftype.currentIndex()
        ...
        def copy_to_main_cmbbk(value):
            i = self.plot_dialog.comboBox.currentIndex()
            cmb = self.findChild(QtGui.QComboBox, params.xanes[i].objectName() + '_cmb')
        ...
        def copy_to_all_data():
            for cb in params.xanes:
                ...
                self.findChild(QtGui.QComboBox, cb.objectName() + '_cmb').setCurrentIndex(self.plot_dialog.comboBox_Ftype.currentIndex())
        ...
        params.cBs_BL14 = [self.u.BL14_ch1,self.u.BL14_ch2,self.u.BL14_ch3,self.u.BL14_ch4,self.u.BL14_ch5,self.u.BL14_ch6,
                           self.u.BL14_ch7,self.u.BL14_ch8,self.u.BL14_ch9,self.u.BL14_ch10,self.u.BL14_ch11,self.u.BL14_ch12,
                           self.u.BL14_ch13,self.u.BL14_ch14,self.u.BL14_ch15,self.u.BL14_ch16,self.u.BL14_ch17,self.u.BL14_ch18,
                           self.u.BL14_ch19,self.u.BL14_ch20,self.u.BL14_ch21,self.u.BL14_ch22,self.u.BL14_ch23,self.u.BL14_ch24,
                           self.u.BL14_ch25,self.u.BL14_ch26,self.u.BL14_ch27,self.u.BL14_ch28,self.u.BL14_ch29,self.u.BL14_ch30,
                           self.u.BL14_ch31, self.u.BL14_ch32, self.u.BL14_ch33, self.u.BL14_ch34,self.u.BL14_ch35, self.u.BL14_ch36]

        params.cBs_BL14_checkstates = []
        params.cBs_ExMode_checkstates = []

        params.cBs_ExMode = [self.u.Couter1,self.u.Couter2,self.u.Couter3,self.u.Couter4,
                             self.u.Couter5,self.u.Couter6,self.u.Couter7]
        self.u.Couter8.setEnabled(False)
        self.u.Couter9.setEnabled(False)

        for cb in params.cBs_BL14:
            params.cBs_BL14_checkstates.append(QtCore.Qt.Checked)

        for cb in params.cBs_ExMode:
            params.cBs_ExMode_checkstates.append(QtCore.Qt.Checked)

        def record_CheckState():
            params.cBs_BL14_checkstates =[]
            for cb in params.cBs_BL14:
                params.cBs_BL14_checkstates.append(cb.checkState())
        ...
        params.BL14_ShapingTimes = ["no correction","C:0.5 us"]
        self.u.BL14_ST.addItems(params.BL14_ShapingTimes)
        self.u.BL14_ST.setCurrentIndex(0)
        self.u.BL14_Edge.addItems(["K","L"])
        self.u.BL14_rB_REX.toggle()

        fig_BL14_1 = Figure(figsize=(320, 320), dpi=72, facecolor=(1, 1, 1), edgecolor=(0, 0, 0))
        ax_BL14_1 = fig_BL14_1.add_subplot(111)
        ax_BL14_1.set_xlabel("E / eV")
        ax_BL14_1.set_ylabel("$\mu$ t")
        canvas_BL14_1 = FigureCanvas(fig_BL14_1)
        self.u.BL14_widget1.setLayout(params.grid6)
        BL14_navibar_1 = NavigationToolbar(canvas_BL14_1, self.u.BL14_widget1)
        params.grid6.addWidget(canvas_BL14_1, 0, 0)
        params.grid6.addWidget(BL14_navibar_1)

        fig_BL14_2 = Figure(figsize=(320, 320), dpi=72, facecolor=(1, 1, 1), edgecolor=(0, 0, 0))
        ax_BL14_2 = fig_BL14_2.add_subplot(111)
        ax_BL14_2.set_xlabel("E / eV")
        ax_BL14_2.set_ylabel("$\mu$ t")
        canvas_BL14_2 = FigureCanvas(fig_BL14_2)
        self.u.BL14_widget2.setLayout(params.grid7)
        BL14_navibar_2 = NavigationToolbar(canvas_BL14_2, self.u.BL14_widget2)
        params.grid7.addWidget(canvas_BL14_2, 0, 0)
        params.grid7.addWidget(BL14_navibar_2)

        scroll_layout_BL14 = QtGui.QVBoxLayout()
        scroll_widgets_BL14 = QtGui.QWidget()
        scroll_widgets_BL14.setLayout(scroll_layout_BL14)
        self.u.scrollArea_17.setWidget(scroll_widgets_BL14)

        def plot_each_ch_BL14():
            conf = cwd + "/" + "BL14.conf"
            str_tconst = open(conf).read()
            DT = yaml.load(str_tconst)
            sum = np.zeros(len(params.Energy))
            # while len(ax_BL14_1.lines) != 0:
            #     ax_BL14_1.lines.pop(0).remove
            # while len(ax_BL14_2.lines) != 0:
            #     ax_BL14_2.lines.pop(0).remove
            ax_array = return_new_axis(params.grid6)
            ax_BL14_1 = ax_array[0]
            ax_array = return_new_axis(params.grid7)
            ax_BL14_2 = ax_array[0]

            for axis in [ax_BL14_1,ax_BL14_2]:
            ...
            if self.u.BL14_ST.currentText() == "no correction":
                for cb in params.cBs_BL14:
                    ...
                    if cb.isChecked():
                        ...
                        sum = np.add(sum, np.array(params.darray[params.cBs_BL14.index(cb)]))
                ...
                for cb in params.cBs_BL14:
                    ...
                    if cb.isChecked():
                        ...
                        ax_BL14_1.plot(params.Energy, ut, color=params.colours6[params.cBs_BL14.index(cb)%6])
            ax_BL14_2.plot(params.Energy, np.divide(sum, params.i0),color = "red")
            canvas_BL14_1.draw()
            canvas_BL14_2.draw()


        def read_dat_BL14(Test_Dat):
            params.ignore_or_not = []
            params.angles = []
            params.aq_time = []
            params.i0 = []
            ...
            while k < 36:
                ...
                #print params.cbs[k].checkState()
                    params.cBs_BL14[k].setCheckState(QtCore.Qt.CheckState.Checked)
                elif params.ignore_or_not[k] == "0":
                    params.cBs_BL14[k].setEnabled(False)
                k += 1
            if self.u.cB_keep_condition_2.isChecked():
                for cb in params.cBs_BL14:
                    cb.setCheckState(params.cBs_BL14_checkstates[params.cBs_BL14.index(cb)])
        ...
        def read_dat_BL14_for_SaveMany(Test_Dat):
            ignore_or_not = []
            angles = []
            aq_time = []
            i0 = []
        ...
        def select_or_release_all_BL14():
            checked_cb = []
            for cb in params.cBs_BL14:
                if cb.isChecked():
                    checked_cb.append(cb)
        ...
        def plot_BL14():
            ...
            for cb in params.cBs_BL14:
                cb.setEnabled(True)
                cb.setCheckState(QtCore.Qt.CheckState.Unchecked)
            for t_rb in params.d_rbs_BL14:
                if t_rb.isChecked():
                    params.current_dfile = params.dir + "/" + t_rb.objectName()
                    if re.match(r"(.+)\.\d+(\.dat)?", t_rb.objectName()) is None:
                        #params.current_ofile = o_dir + "/" + t_rb.objectName() + "_000" + ".ex3"
                        break
                    elif re.match(r"(.+)\.(\d+)(\.dat)?", t_rb.objectName()):
                        t_line = t_rb.objectName().split(".")
                        #params.current_ofile =o_dir + "/" + t_line[0] + "_" + t_line[1]  + ".ex3"
                        break
            read_dat_BL14(params.current_dfile)
            plot_each_ch_BL14()

        def func_for_rb_BL14():
            plot_BL14()
            self.u.BL14_RSall.setText("Release all")

        def openFiles_BL14():
            while scroll_layout_BL14.count() > 0:
                b = scroll_layout_BL14.takeAt(len(params.d_rbs_BL14) - 1)
                params.dfiles_BL14.pop()
                params.d_rbs_BL14.pop()
                b.widget().deleteLater()
            dat_dir = home_dir.homePath()
            if params.dir == "":
                dat_dir = home_dir.homePath()
            ...
            for fname in files[0]:
                info = QtCore.QFileInfo(fname)
                params.dfiles_BL14.append(info.fileName())
            for d_file in params.dfiles_BL14:
                rb = QtGui.QRadioButton(d_file)
                rb.setObjectName(d_file)
                params.d_rbs_BL14.append(rb)
                scroll_layout_BL14.addWidget(rb)
            for t_rb in params.d_rbs_BL14:
                t_rb.toggled.connect(func_for_rb_BL14)
            #print scroll_layout_BL14.count()
            params.d_rbs_BL14[0].toggle()

        def define_outdir_BL14():
            self.u.BL14_textBrowser.clear()
            FO_dialog = QtGui.QFileDialog(self)
            params.outdir = FO_dialog.getExistingDirectory(parent=None, dir=params.dir)
            self.u.BL14_textBrowser.append(params.outdir)
            #if self.u.textBrowser_2.toPlainText() == "":
            #    self.u.textBrowser_2.append(params.outdir)
        ...
        def Save_BL14():
            ...
            o_dir = params.dir
            exd = ""
            if self.u.BL14_rB_REX.isChecked():
                exd = ".ex3"
            else:
                exd = "_a.dat"
            if params.outdir == "":
            ...
            if self.u.BL14_rB_REX.isChecked():
                line = "[EX_DATA]\n*DATE=\n*EX_SAMPLE=\n"
                atom = "*EX_ATOM=" + self.u.BL14_Atom.text() + "\n"
                edge = "*EX_EDGE=" + self.u.BL14_Edge.currentText() + "\n"
                line2 = "*EX_COMMENT=\n*EX_GONIO=\n*EX_ATTACHIMENT=\n*EX_TARGET=\n*EX_FILAMENT=\n*EX_MEASURE=\n*EX_I0_DETECTOR=\n*EX_I_DETECTOR=\n*EX_CRYSTAL=\n*EX_2D=\n*EX_KV=\n*EX_MA=\n*EX_SLIT_DS=\n*EX_SLIT_RS=\n*EX_SLIT_H=\n*EX_REPEAT=\n*EX_AREA1=\n*EX_AREA2=\n*EX_AREA3=\n*EX_AREA4=\n*EX_AREA5=\n"
                line3 = "\n[EX_BEGIN]\n"
                out.write(line + atom + edge + line2 + line3)
            else:
        ...
        def Save_all_as_Current_BL14():
            ...
            if self.u.BL14_rB_REX.isChecked():
                ...
                exd = '_a.dat'
            if self.u.BL14_textBrowser.toPlainText() != '':
                o_dir = os.path.abspath(self.u.BL14_textBrowser.toPlainText())
            for t_rb in params.d_rbs_BL14:
                # sum = np.zeros(len(params.Energy))
                Energy, aq_time, i0, darray, ICR = read_dat_BL14_for_SaveMany(params.dir + "/" + t_rb.objectName())
                sum = np.zeros(len(Energy))
        ...
        self.u.BL14_Open.clicked.connect(openFiles_BL14)
        self.u.BL14_RSall.clicked.connect(select_or_release_all_BL14)
        self.u.BL14_outpath.clicked.connect(define_outdir_BL14)
        self.u.BL14_ST.currentIndexChanged.connect(plot_each_ch_BL14)
        self.u.BL14_Save.clicked.connect(Save_BL14)
        self.u.pB_save_all_SSD_2.clicked.connect(Save_all_as_Current_BL14)
        for cb in params.cBs_BL14:
            cb.clicked.connect(plot_each_ch_BL14)
            cb.clicked.connect(record_CheckState)

        ####################For Expert Mode####################
        self.u.ExMode_rB_REX.toggle()

        fig_ExMode_1 = Figure(figsize=(320, 320), dpi=72, facecolor=(1, 1, 1), edgecolor=(0, 0, 0))
        ax_ExMode_1 = fig_ExMode_1.add_subplot(111)
        ax_ExMode_1.set_xlabel("E / eV")
        ax_ExMode_1.set_ylabel("$\mu$ t")
        canvas_ExMode_1 = FigureCanvas(fig_ExMode_1)
        self.u.ExMode_widget1.setLayout(params.grid8)
        ExMode_navibar_1 = NavigationToolbar(canvas_ExMode_1, self.u.ExMode_widget1)
        params.grid8.addWidget(canvas_ExMode_1, 0, 0)
        params.grid8.addWidget(ExMode_navibar_1)

        fig_ExMode_2 = Figure(figsize=(320, 320), dpi=72, facecolor=(1, 1, 1), edgecolor=(0, 0, 0))
        ax_ExMode_2 = fig_ExMode_2.add_subplot(111)
        ax_ExMode_2.set_xlabel("E / eV")
        ax_ExMode_2.set_ylabel("$\mu$ t")
        canvas_ExMode_2 = FigureCanvas(fig_ExMode_2)
        self.u.ExMode_widget2.setLayout(params.grid9)
        ExMode_navibar_2 = NavigationToolbar(canvas_ExMode_2, self.u.ExMode_widget2)
        params.grid9.addWidget(canvas_ExMode_2, 0, 0)
        params.grid9.addWidget(ExMode_navibar_2)

        scroll_layout_ExMode = QtGui.QVBoxLayout()
        scroll_widgets_ExMode = QtGui.QWidget()
        scroll_widgets_ExMode.setLayout(scroll_layout_ExMode)
        self.u.ExMode_scrollArea.setWidget(scroll_widgets_ExMode)

        for cb in params.cBs_ExMode:
            cb.setAutoFillBackground(True)
            plt = cb.palette()
            plt.setColor(cb.backgroundRole(), params.colors[params.cBs_ExMode.index(cb)])
            plt.setColor(cb.foregroundRole(), "#FFFFFF")
            cb.setPalette(plt)
            palette.setColor(cb.foregroundRole(), "Grey")
            # if params.cBs_ExMode.index(cb)
            #     plt.setColor(cb.backgroundRole(), params.rgb_color[0])
            # elif params.cbs.index(cb) >= 3 and params.cbs.index(cb) <=6:
            #     plt.setColor(cb.backgroundRole(), params.rgb_color[1])
            # elif params.cbs.index(cb) >= 7 and params.cbs.index(cb) <=11:
        ...
        def plot_each_ch_ExMode():
            # conf = cwd + "/" + "BL14.conf"
            # str_tconst = open(conf).read()
            # DT = yaml.load(str_tconst)
            sum = np.zeros(len(params.Energy))
            ax_array = return_new_axis(params.grid8)
            ax_ExMode_1 = ax_array[0]
            ax_array = return_new_axis(params.grid9)
            ax_ExMode_2 = ax_array[0]

            for axis in [ax_ExMode_1, ax_ExMode_2]:
                axis.set_xlabel("E / eV")
                axis.set_ylabel("$\mu$ t")
                axis.relim()
                axis.autoscale_view(True, True, True)
            ...
            for cb in params.cBs_ExMode:
                print 'Hi'
                if cb.isChecked():
                    ut = np.divide(np.array(params.darray[params.cBs_ExMode.index(cb)]), np.array(params.i0))
                    sum = np.add(sum, np.array(params.darray[params.cBs_ExMode.index(cb)]))
                    ax_ExMode_1.plot(params.Energy, ut, color=params.colors[params.cBs_ExMode.index(cb)])

            ax_ExMode_2.plot(params.Energy, np.divide(sum, params.i0), color="red")
            canvas_ExMode_1.draw()
            canvas_ExMode_2.draw()

        def read_dat_ExMode(Test_Dat):
            params.ignore_or_not = []
            params.angles = []
            params.aq_time = []
            params.i0 = []
        ...
        def read_dat_ExMode_for_SaveMany(Test_Dat):
            angles = []
            aq_time = []
            i0 = []
            dat = []
        ...
        def select_or_release_all_ExMode():
            checked_cb = []
            for cb in params.cBs_ExMode:
                if cb.isChecked():
                    checked_cb.append(cb)
            if len(checked_cb) > 0:
                self.u.ExMode_releaseAll.setText("Select All")
                for cb in checked_cb:
                    cb.setCheckState(QtCore.Qt.CheckState.Unchecked)
            elif len(checked_cb) == 0:
                self.u.ExMode_releaseAll.setText("Release All")
                k = 0
                print len(params.ignore_or_not)
                while k < 36:
                    if params.ignore_or_not[k] != "0":
                        params.cBs_ExMode[k].setCheckState(QtCore.Qt.CheckState.Checked)
                    elif params.ignore_or_not[k] == "0":
                        params.cBs_ExMode[k].setEnabled(False)
                    k += 1
            plot_each_ch_BL14()
        ...
        def plot_ExMode():
            ...
            for cb in params.cBs_ExMode:
                cb.setEnabled(True)
                cb.setCheckState(QtCore.Qt.CheckState.Unchecked)
            for t_rb in params.d_rbs_ExMode:
                if t_rb.isChecked():
                    params.current_dfile = params.dir + "/" + t_rb.objectName()
                    if re.match(r"(.+)\.\d+(\.dat)?", t_rb.objectName()) is None:
                        # params.current_ofile = o_dir + "/" + t_rb.objectName() + "_000" + ".ex3"
                        ...
                        # params.current_ofile =o_dir + "/" + t_line[0] + "_" + t_line[1]  + ".ex3"
                        break
            read_dat_ExMode(params.current_dfile)
            plot_each_ch_ExMode()

        def func_for_rb_ExMode():
            plot_ExMode()
            self.u.ExMode_releaseAll.setText("Release all")

        def openFiles_ExMode():
            self.u.ExMode_OUTPATH.clear()
            while scroll_layout_ExMode.count() > 0:
                b = scroll_layout_ExMode.takeAt(len(params.d_rbs_ExMode) - 1)
                params.dfiles_ExMode.pop()
                params.d_rbs_ExMode.pop()
                b.widget().deleteLater()
            dat_dir = home_dir.homePath()
            if params.dir == "":
                dat_dir = home_dir.homePath()
            ...
            finfo = QtCore.QFileInfo(files[0][0])
            params.dir = finfo.path()
            self.u.ExMode_OUTPATH.append(params.dir)
            for fname in files[0]:
                info = QtCore.QFileInfo(fname)
                params.dfiles_ExMode.append(info.fileName())
            for d_file in params.dfiles_ExMode:
                rb = QtGui.QRadioButton(d_file)
                rb.setObjectName(d_file)
                params.d_rbs_ExMode.append(rb)
                scroll_layout_ExMode.addWidget(rb)
            for t_rb in params.d_rbs_ExMode:
                t_rb.toggled.connect(func_for_rb_ExMode)
            # print scroll_layout_BL14.count()
            params.d_rbs_ExMode[0].toggle()

        def define_outdir_ExMode():
            self.u.BL14_textBrowser.clear()
            FO_dialog = QtGui.QFileDialog(self)
            params.outdir = FO_dialog.getExistingDirectory(parent=None, dir=params.dir)
            self.u.ExMode_OUTPATH.append(params.outdir)
            # if self.u.textBrowser_2.toPlainText() == "":
            #    self.u.textBrowser_2.append(params.outdir)
        ...
        def Save_ExMode():
            ...
            o_dir = params.dir
            exd = ""
            if self.u.ExMode_rB_REX.isChecked():
                exd = ".ex3"
            else:
                exd = "_a.dat"
            if params.outdir == "":
            ...
            for cb in params.cBs_ExMode:
                if cb.isChecked():
                    # ut = np.divide(np.array(params.darray[params.cBs_BL14.index(cb)]), np.array(params.i0))
                    sum += params.darray[params.cBs_ExMode.index(cb)]
                    ax_ExMode_1.plot(params.Energy, ut, color=params.colors[params.cBs_ExMode.index(cb)])
            ut = np.divide(sum, params.i0)
            k = 0
            while k < len(params.Energy):
        ...
        def Save_all_as_Current_ExMode():
            # conf = cwd + "/" + "BL14.conf"
            # str_tconst = open(conf).read()
            # DT = yaml.load(str_tconst)
            sum = np.zeros(len(params.Energy))
            ...
            if self.u.ExMode_rB_REX.isChecked():
                ...
                exd = '_a.dat'
            if self.u.ExMode_OUTPATH.toPlainText() != '':
                o_dir = os.path.abspath(self.u.ExMode_OUTPATH.toPlainText())
            for t_rb in params.d_rbs_ExMode:
                # sum = np.zeros(len(params.Energy))
                Energy, aq_time, i0, darray = read_dat_ExMode_for_SaveMany(params.dir + "/" + t_rb.objectName())
                sum = np.zeros(len(Energy))
                if re.match(r"(.+)(\.|_)\d+(\.dat)?", t_rb.objectName()) is None:
                    current_ofile = o_dir + "/" + t_rb.objectName() + "_000" + exd
                elif re.match(r"(.+)(\.|_)(\d+)(\.dat)?", t_rb.objectName()):
        ...
        def record_CheckState_ExMode():
            params.cBs_ExMode_checkstates =[]
            for cb in params.cBs_ExMode:
                params.cBs_ExMde_checkstates.append(cb.checkState())

        self.u.ExMode_OpenFiles.clicked.connect(openFiles_ExMode)
        self.u.ExMode_releaseAll.clicked.connect(select_or_release_all_ExMode)
        self.u.ExMode_setOutPath.clicked.connect(define_outdir_ExMode)
        #self.u.comboBox_14.currentIndexChanged.connect(plot_each_ch_ExMode)
        self.u.ExMode_Save.clicked.connect(Save_ExMode)
        self.u.ExMode_SaveAll.clicked.connect(Save_all_as_Current_ExMode)
        for cb in params.cBs_ExMode:
            cb.clicked.connect(plot_each_ch_ExMode)
            cb.clicked.connect(record_CheckState_ExMode)
